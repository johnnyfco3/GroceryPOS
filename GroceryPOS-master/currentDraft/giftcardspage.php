...
<head>
  <title>Customers with Gift Cards | MarketPOS</title>
...
<!--END nav sidebar-->
  ...
  <div id="content">
          ...
          <ul class="nav navbar-nav mr-auto">
            <div class="navbar-brand">
              <a href="customercontrol.php"><svg width=".8em" height=".8em" viewBox="0 0 16 16" class="bi bi-people-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1H7zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm-5.784 6A2.238 2.238 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.325 6.325 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1h4.216zM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z" />
                </svg> Customers</a><span class="px-2"> > </span><a href="giftcardspage.php"><span>Gift Cards</span></a>
            </div>
    ...
    <nav class="navbar navbar-light" id="salespanel">
      <!--  <form class="form-inline" method="post" action="giftcardspage.php">
        <div class="nav-item px-3">
          <input class="form-control" size="10" name="giftcard" placeholder="Search Gift Cards">
          <button class="btn btn-dark navbar-btn" name="giftsearch"> Search</button>
        </div>
        <div class="nav-item px-3">
          <input class="form-control" size="20" name="customer" placeholder="Search Customers">
          <button class="btn btn-dark navbar-btn" name="submit-search"> Search</button>
        </div>
        </form> -->
      <div class="nav-item mr-auto">
        <a class="btn navbar-btn btn-dark" href="newgiftcard.php" role="button"> New Gift Card</a></div>
      ...
      <div class="row px-4 justify-content-center">
        <table class="table table-bordered table-hover" style="font-size:90%;">
          ...
          <tbody class="bg-white">
            ...
            /*  if (isset($_POST['giftsearch'])) {
              $search = mysqli_real_escape_string($conn, $_POST['giftcard']);
              $query = "SELECT * FROM gift_card WHERE gift_id LIKE '%$search%'";
              $result = mysqli_query($conn, $query);
              $queryResults = mysqli_num_rows($result);
              if ($queryResults = 0) {
                echo "<div class='row mt-3'>There are 0 results matching your search</div><br>";
              } else { */
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              $tquery = "SELECT customer_id FROM ticket_system WHERE ticket_id LIKE '%$ticketID%'";
              $tresult = mysqli_query($conn, $tquery);
              $tRow = mysqli_fetch_assoc($tresult);
              $customerID = $tRow['customer_id'];
              $cquery = "SELECT first_name, last_name FROM customer_info WHERE customer_id LIKE '%$customerID%'";
              $cresult = mysqli_query($conn, $cquery);
              $cRow = mysqli_fetch_assoc($cresult);
              echo "
                  <tr>
                  <td>" . $row['gift_id'] . "</td>
                  <td>" . $cRow['first_name'] . " " . $cRow['last_name'] . "</td>
                  <td>" . $row['ticket_id']  . "</td>
